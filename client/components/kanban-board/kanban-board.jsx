...
class KanbanBoard extends Component {
  ...
  render() {
    const ReallyAwfulNestedStoreWorkaround = props => (
    ...
    return (
      <div>
        <Board data={this.createBoard()} draggable handleDragEnd={this.handleDragEnd} customCardLayout>
          <ReallyAwfulNestedStoreWorkaround />
        </Board>
      </div>
    );
  }
...
const mapState = ({ issues, collaborators, labels }) => {
  ...
  const inProgressIssues = issueList.filter(issue =>
    issue.labels.find(label => label.name === "in progress")
  );
